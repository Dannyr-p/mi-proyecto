...
function cargarSubespecialidadesParaTemas() {
  ...
  }, (err) => mostrarEstado("Error cargando subespecialidades para temas: " + err));
}
...
function cargarTemas() {
  ...
  if (!especialidadId) return;
  db.collection("temas").where("especialidadId", "==", especialidadId).orderBy("nombre").onSnapshot((snapshot) => {
    temaSelect.innerHTML = "<option value=''>Seleccione tema</option>";
    snapshot.forEach((doc) => {
      const tema = doc.data();
    ...
    listarTemas();
  }, (err) => mostrarEstado("Error cargando temas: " + err));
}
...
function listarTemas() {
  ...
  if (!especialidadId) { cont.innerHTML = ""; return; }
  db.collection("temas").where("especialidadId", "==", especialidadId).orderBy("nombre").get().then(snapshot => {
    let html = "<ul>";
    snapshot.forEach(doc => {
      const tema = doc.data();
      if (!subespecialidadId || !tema.subespecialidadId || tema.subespecialidadId === subespecialidadId) {
